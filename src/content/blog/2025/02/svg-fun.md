...
It's been a while that SVGs fascinated me, I think vector graphics caught my attention during the Flash era but I only gave them a chance around 2012 when I was trying to play with filters and animations for a web based D&D table top project.
...
```html
<a href="#">
	<svg viewBox="0 0 10 10" aria-labeledby="icon-title">
...
That super power is the reason the logos on this website are inlined to the HTML instead of image files. It allows the logo to change color and style based on the theme and be interactive as other elements in the page, having hover and active styles, for example.
...
That happens to be exactly the case! The different logo sizes are based on one another, many of the elements repeat themselves! It is a win! Horray!

One interesting thing about the `<use>` tag is that you are not limited to only, well, _using_ the thing, you can also change it in some ways, like moving around, scaling, rotating, and changing colors.

This solves the problem of the logos getting out of hand and excessively verbose (specially with specific logos for some themes). Back to the responsiveness problem...
...
Nothing prohibits me from not adding a `viewBox` attribute to the SVG, but doing so makes it not display correctly, the size of the SVG viewport is inferred from some browser fallback logic that makes things display weirdly and be all in different places.
...
So, after a long, long exploration on how SVGs work and how to make cool things with it things magically worked and I got cool logos to accompany cool themes!
